...
"""
Smart Budget Enforcer - Enhanced FastAPI Service
UPDATED: Now includes email-optimized AI recommendations for breach notifications
"""
...
# Enhanced Pydantic models for email integration
class BudgetDataRequest(BaseModel):
    """Budget data structure optimized for email recommendations"""
    id: str
    name: str
    department: str
    category: str
...
class ExpenseDataRequest(BaseModel):
    """Expense data structure for email recommendations"""
    id: str
    amount: float
    department: str
    category: str
...
class BreachContextRequest(BaseModel):
    """Enhanced breach context for email notifications"""
    type: str
    severity: str
    usage_percentage: float
    overage_amount: float
    triggered_by_expense: float
    email_notification_required: Optional[bool] = True

class EmailOptimizedRecommendationRequest(BaseModel):
    """Enhanced request structure for email-optimized recommendations"""
    budget_data: BudgetDataRequest
    expense_data: ExpenseDataRequest
    breach_context: BreachContextRequest
    user_id: str
    email_optimization: Optional[bool] = True
...
async def lifespan(app: FastAPI):
    ...
    try:
        logger.info("🚀 Starting Enhanced Email-Integrated LangChain service...")
        ...
        logger.info("✅ Enhanced Email-Integrated LangChain service started successfully")
    ...
    logger.info("🛑 Shutting down Enhanced Email-Integrated LangChain service...")

# Initialize FastAPI app
app = FastAPI(
    title="Smart Budget Enforcer - Email-Integrated AI Service",
    description="AI-powered budget processing with email-optimized recommendations for breach notifications",
    version="3.3.0",
    lifespan=lifespan
)
...
async def root():
    """Root endpoint - service status"""
    return {
        "service": "Smart Budget Enforcer - Email-Integrated LangChain + LangGraph Service",
        "status": "running",
        "version": "3.3.0",
        "implementation": "email_optimized_langchain_agents",
        "features": [
            "Email-Optimized AI Recommendations",
            "Breach Notification Integration",
            "Multi-format Document Support",
            "Real-time Recommendation Generation",
            "HTML Email Content Generation"
        ],
        "email_integration": "fully_enabled",
        "endpoints": {
            "health": "/health",
            "process_document": "/process-document",
            "generate_recommendations": "/generate-recommendations",
            "email_optimized_recommendations": "/generate-email-recommendations"
        },
        "docs": "/docs"
    }
...
async def health_check():
    ...
    try:
        ...
        return HealthResponse(
            status="healthy",
            service="Smart Budget Enforcer - Email-Integrated LangChain Service",
            agents_loaded=workflow_status.get("workflow_ready", False),
            vector_db_connected=True,
            version="3.3.0"
        )
...
def validate_workflow():
    ...
    if workflow_instance is None:
        raise HTTPException(
            status_code=503,
            detail="Email-Integrated LangChain workflow not initialized. Please wait and try again."
        )
...
async def process_document(
    ...
    """Enhanced document processing with email integration support"""
    start_time = datetime.now()
    temp_file_path = None
    
    try:
        logger.info(f"📄 Processing document with email integration: {file.filename} for user: {user_id}")
        ...
        # Process through Enhanced LangChain workflow
        try:
            workflow = get_workflow()
            result = workflow.process_document_upload(temp_file_path, user_id)
            ...
            logger.error(f"❌ Enhanced LangChain workflow error: {e}")
            raise HTTPException(status_code=422, detail=f"Enhanced document processing failed: {str(e)}")
        ...
        if not result.get("success", False):
            error_msg = result.get("error", "Unknown processing error")
            logger.error(f"❌ Enhanced workflow failed: {error_msg}")
            raise HTTPException(status_code=422, detail=error_msg)
        ...
        if not budget_data:
            raise HTTPException(
                status_code=422,
                detail="No budget data could be extracted from this document."
            )
        
        # Enhanced response with email integration metadata
        response_data = {
            "success": True,
            "budget_count": len(budget_data),
            "budget_data": budget_data,
            "processing_time": processing_time,
            "processing_steps": result.get("processing_steps", []),
            "message": f"Successfully processed document and extracted {len(budget_data)} budget items with email integration support",
            "email_integration": {
                "enabled": True,
        ...
        logger.info(f"✅ Enhanced processing complete: {len(budget_data)} items extracted with email support")
        return JSONResponse(content=response_data)
...
@app.post("/generate-recommendations")
async def generate_recommendations(
    request: EmailOptimizedRecommendationRequest,
    _: None = Depends(validate_workflow)
):
    """Enhanced recommendation generation with email optimization"""
    start_time = datetime.now()
    
    try:
        logger.info(f"🧠 Generating email-optimized recommendations for user: {request.user_id}")
        logger.info(f"📧 Email optimization enabled: {request.email_optimization}")
        ...
        if result["success"]:
            ...
            # Enhanced response with email optimization metadata
            response_data = {
                "success": True,
                "message": f"Generated {len(recommendations)} email-optimized AI recommendations",
                "recommendations": recommendations,
                "processing_time": processing_time,
                "email_optimization": {
                    "enabled": request.email_optimization,
                    "recommendations_count": len(recommendations),
                    "email_ready": all(rec.get('email_optimized', False) for rec in recommendations),
                ...
                "processing_info": {
                    "ai_model": "gemini-1.5-flash",
                    "processing_method": "email_optimized_langchain_agents",
                    "agent_count": 5,
                    "email_integration": "fully_enabled",
                    "confidence_level": result.get("analysis_metadata", {}).get("confidence_level", "Medium")
                }
            }
            
            logger.info(f"✅ Email-optimized recommendations generated: {len(recommendations)}")
            return JSONResponse(content=response_data)
        else:
            error_msg = result.get("error", "Unknown error")
            logger.error(f"❌ Email-optimized recommendation generation failed: {error_msg}")
        ...
        processing_time = (datetime.now() - start_time).total_seconds()
        logger.error(f"❌ Unexpected error in email-optimized recommendation generation: {e}")
        
        # Enhanced fallback with email optimization
        fallback_recommendations = [{
            "title": "Email-Integrated Budget Review Required",
            "description": "Our email-integrated AI system is temporarily unavailable. A manual budget review is recommended with email notification to stakeholders once analysis is complete.",
            "type": "approval_request",
            "priority": 2,
            "estimated_savings": 0,
            "implementation_timeline": "24-48 hours",
            "implementation_steps": [
                "Schedule manual budget review meeting",
                "Prepare budget analysis presentation",
                "Send email notification to stakeholders",
                "Implement recommended actions from review"
            ],
            "success_metrics": [
                "Manual review completed within 48 hours",
                "All stakeholders notified via email",
                "Action plan implemented successfully"
            ],
            "risk_factors": [
                "Manual process delays: Medium risk",
                "Communication gaps: Low risk with email follow-up"
            ],
            ...
            "fallback_reason": "Email-integrated AI analysis temporarily unavailable"
        }]
        ...
        return JSONResponse(
            ...
            content={
                "success": True,
                "message": "Generated email-optimized fallback recommendations",
                "recommendations": fallback_recommendations,
                "fallback_used": True,
                "email_optimization": {
                    "enabled": True,
                ...
                "error_context": str(e),
                "processing_time": processing_time,
                "processing_method": "email_optimized_fallback_system"
            }
        )

@app.post("/generate-email-recommendations")
async def generate_email_recommendations(
    request: EmailOptimizedRecommendationRequest
):
    """Dedicated endpoint for email-optimized recommendations"""
    try:
        # Force email optimization
        request.email_optimization = True
        
        # Call the main recommendation endpoint
        return await generate_recommendations(request)
        
    except Exception as e:
        logger.error(f"❌ Email recommendation endpoint error: {e}")
        raise HTTPException(status_code=500, detail=f"Email recommendation generation failed: {str(e)}")

@app.get("/workflow/status")
async def get_workflow_status():
    """Get Enhanced workflow status with email integration info"""
    try:
        workflow = get_workflow()
        if workflow:
            status = workflow.get_workflow_status()
            status.update({
                "implementation": "email_integrated_langchain_with_langgraph",
                "agent_framework": "email_optimized_langchain_tools",
                "orchestration": "langgraph_state_machine",
                "ai_reasoning": "gemini_1.5_flash_email_enhanced",
                "email_integration": {
                    "enabled": True,
                    "recommendation_optimization": True,
                    "breach_notification_support": True,
            ...
            return JSONResponse(content={"workflow_ready": False, "error": "Email-integrated workflow not initialized"})
    
    except Exception as e:
        logger.error(f"❌ Error getting enhanced workflow status: {e}")
        raise HTTPException(status_code=500, detail=f"Workflow status error: {str(e)}")
...
async def global_exception_handler(request, exc):
    """Global exception handler"""
    logger.error(f"❌ Unhandled exception in Email-Integrated service: {exc}")
    return JSONResponse(
        status_code=500,
        content={
            "success": False,
            "error": "Internal server error",
            "detail": str(exc) if DEBUG else "An unexpected error occurred in Email-Integrated LangChain service",
            "service_type": "email_integrated_langchain_agents",
            "email_integration": "enabled_but_error_occurred"
        }
    )
